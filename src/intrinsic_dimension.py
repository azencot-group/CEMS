...
def intrinsic_dimension(X, verbose=False):
    # if X in numpy use intrinsic_dimension_np
    if isinstance(X, np.ndarray):
        est_id = intrinsic_dimension_np(X, verbose=verbose)
    elif isinstance(X, torch.Tensor):
        est_id = intrinsic_dimension_torch(X, verbose=verbose)
    else:
        raise ValueError("Input must be a numpy array or a torch tensor.")
    return est_id


def intrinsic_dimension_np(X, verbose=False):
    p_dist = pdist(X, metric='euclidean')
    dist = squareform(p_dist)
    est_np = estimate_np(dist, verbose=verbose)
    return est_np


def intrinsic_dimension_torch(X, verbose=False):
    try:
        dists = torch.cdist(X, X, p=2, compute_mode='donot_use_mm_for_euclid_dist')
        est_torch = estimate_torch(dists, verbose=verbose)
    except RuntimeError as e:
        if "out of memory" in str(e):
            print("Out of memory error encountered. Trying batched estimation.")
            # Fallback to batched estimation
            est_torch = estimate_torch_batched(X, verbose=verbose)
        else:
            raise e
    return est_torch
